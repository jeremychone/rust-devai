...
use crate::script::rhai_script::rhai_modules::{
	rhai_cmd, rhai_file_md, rhai_html, rhai_json, rhai_path, rhai_rust, rhai_web,
};
use crate::Result;
use rhai::Engine;
...
pub fn new_rhai_engine(runtime_context: RuntimeContext) -> Result<Engine> {
	...
	let mut rhai_file_module = rhai_file::rhai_module(&runtime_context);
	rhai_file_module.merge(&rhai_file_md::rhai_module(&runtime_context));

	engine.register_static_module("file", rhai_file_module.into());
	...
	engine.register_static_module("cmd", rhai_cmd::rhai_module().into());
	engine.register_static_module("json", rhai_json::rhai_module().into());
